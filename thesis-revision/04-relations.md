...
\((D_\Psi)\) 只在需要拟范数收敛时使用；\((M_{\Psi_1})\) 则专门用于二阶尾量到一阶尾量的转换。
...
已有文献已经在共轭 Young 函数双方有限幂时建立较一般的 Hardy–Orlicz 对偶。因此，普通平方函数部分主要定位为幂型复合条件下的自足证明、投影机制说明和对初稿证明细节的修正。若要把 \(K^\Psi\) 部分称为严格推广，必须给出满足本文全部极大不等式、但不属于已有双方有限幂范围的明确 Young 函数例子。
