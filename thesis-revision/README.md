...
按照 Dam 二次复合情形的原证明顺序，另行生成了 \(r\ge2\) 幂型复合推广的逐步证明：

`引理3.1_r次幂复合条件_完整证明.docx`

对应可维护源稿为 `lemma3.1-detailed.md`，重新生成命令为：

```bash
python3 tools/build_lemma31_docx.py
```

该版本保留单坐标 Orlicz 对偶、有限截断、较大适应序列空间、凸性不等式、Orlicz 范数检验和平移泛函的原证明路线，不采用向量值对偶方法。
